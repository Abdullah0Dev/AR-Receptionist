...
const STAGES_16K: BiquadCoeffs[] = [
  ...
  { b: [0.25990, 0.51981, 0.25990], a: [1.0,  0.0,      0.03960] }, // Q=0.5412
];
...
export class StreamAudioProcessor {
  ...
  pcmToMulaw(base64Pcm: string, mimeType: string): string {
    ...
    if (mimeType.includes("24000")) {
      const filtered = this.applyFilterCascade(samples, STAGES_24K, this.states24k);
      downsampled = new Int16Array(Math.floor(filtered.length / 3));
      for (let i = 0; i < downsampled.length; i++) downsampled[i] = filtered[i * 3];
    } else if (mimeType.includes("16000")) {
      const filtered = this.applyFilterCascade(samples, STAGES_16K, this.states16k);
      downsampled = new Int16Array(Math.floor(filtered.length / 2));
      for (let i = 0; i < downsampled.length; i++) downsampled[i] = filtered[i * 2];
    } else {
      downsampled = samples;
    }

    const mulawBuffer = Buffer.alloc(downsampled.length);
    for (let i = 0; i < downsampled.length; i++) mulawBuffer[i] = encodeSample(downsampled[i]);
...
export class AudioService {
  ...
  static detectVoice(pcmData: Buffer, options: { threshold: number; minEnergy: number }): boolean {
  ...
  static mulawToPcm16k(mulawBase64: string): Buffer {
    ...
    for (let i = 0; i < mulawBuffer.length; i++) pcm8k[i] = decodeSample(mulawBuffer[i]);
